# Proposed Fix for Async Execute Issue

This document describes the proposed fix to make `execute()` properly handle async functions and Promises.

## Summary

The fix modifies `lib/mixins/execute.ts` to:
1. Detect when a script might return a Promise
2. Execute with `returnByValue: false` to get an object reference
3. Check if the result is a Promise (subtype: 'promise')
4. If Promise, use `Runtime.awaitPromise` to get the resolved value
5. Return the resolved value instead of the serialized Promise (`{}`)

## Modified File: `lib/mixins/execute.ts`

Below is the complete modified file with the fix applied:
...
export async function execute(this: RemoteDebugger, command: string, override?: boolean): Promise<any> {
  ...
  if (scriptMightBeAsync) {
    ...
    return await this.executeWithPromiseSupport(command, appIdKey, pageIdKey);
  }
...
### 1. New `mightReturnPromise()` Helper Function

Detects patterns that indicate a script might return a Promise:
...
### 2. Modified `execute()` Function

Now checks if the script might be async and routes to the appropriate execution path:
...
### 3. New `executeWithPromiseSupport()` Function

Handles Promise-returning scripts by:
1. Evaluating with `returnByValue: false` to get object reference
2. Checking if result has `subtype: 'promise'`
3. Using `Runtime.awaitPromise` to get resolved value
4. Falling back gracefully if `awaitPromise` is unavailable

## Backward Compatibility

- Sync scripts continue to work exactly as before
- The async detection is conservative (only detects explicit patterns)
- Falls back gracefully if `Runtime.awaitPromise` is unavailable
...
Run the functional tests (requires iOS Simulator):
```bash
...
### 1. Always Use Runtime.awaitPromise

**Rejected:** Would add unnecessary overhead for sync scripts and might cause issues with non-Promise objects.

### 2. Add a New `executeAsync` Parameter

**Rejected:** Would require changes to all consumers (Appium drivers, test frameworks) rather than fixing at the source.

### 3. Use callAsyncJavaScript API (iOS 14+)

**Considered for future:** This is the cleanest solution but requires changes at the XCUITest driver level, not just appium-remote-debugger. Could be added as an enhancement later.
